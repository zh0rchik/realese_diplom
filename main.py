...
class KlystronAnalyzer:
    ...
    def load_model_parameters(self):
        ...
        try:
            # Загрузка параметров для регрессии
            regression_params_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'regression_params.json')
            with open(regression_params_path, 'r', encoding='utf-8') as f:
                self.regression_params = json.load(f)

            # Загрузка параметров для классификации
    ...
    def create_regression_tab(self):
        ...
        self.yn_metrics = tk.Text(yn_frame, height=35, width=30)
        ...
        self.yc_metrics = tk.Text(yc_frame, height=35, width=30)
        ...
        self.yv_metrics = tk.Text(yv_frame, height=35, width=30)
    ...
    def _train_all_models_thread(self):
        ...
        try:
            # Сначала обучаем все модели регрессии
            for model_type in ['yn', 'yc', 'yv']:
                self._train_regression_thread(model_type)

            # Затем обучаем все модели классификации
            for model_type in ['zn', 'zc', 'zv']:
                self._train_classification_thread(model_type)

            # После обучения всех моделей показываем сообщение
            self.root.after(0, lambda: messagebox.showinfo("Успешно", "Все модели успешно обучены"))
            self.root.after(0, self._hide_loading)

        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Ошибка", f"Ошибка при обучении моделей: {str(e)}"))
            self.root.after(0, self._hide_loading)
    ...
    def show_about(self):
        ...
        about_text = """Анализатор блоков усилителей мощности клистронов\n
            ...
            блоков усилителей мощности клистронов на основе нейросетевых моделей.\n
            Разработчик: [Ваше имя/организация]\n
            © 2023 Все права защищены"""
        messagebox.showinfo("О программе", about_text)

    def show_help(self):
        """Показ инструкции по использованию программы"""
        help_text = """ИНСТРУКЦИЯ ПО ИСПОЛЬЗОВАНИЮ ПРОГРАММЫ\n
            1. Загрузите данные:
               - Данные для регрессии (файл с параметрами x1-x10 и Yн, Yс, Yв)
               - Данные для классификации (файл с Yн, Yс, Yв, U, T и Zн, Zс, Zв)

            2. Обучите модели:
               - Можно обучать модели по отдельности на соответствующих вкладках
               - Или использовать кнопку "Обучить все модели" в меню "Анализ"

            3. Просмотрите метрики моделей:
               - На вкладках "Регрессия" и "Классификация"
               - Или через меню "Анализ" -> "Вывести метрики"

            4. Выполните прогнозирование:
               - Перейдите на вкладку "Прогнозирование"
               - Введите параметры нового клистрона (x1-x10, U, T)
               - Нажмите "Рассчитать прогноз"
               - Для быстрого заполнения можно использовать "Заполнить примерными данными\""""
